...
class CityLandmarkPipeline(object):
    """
    城市地标信息入库
    """
    conn = None
    ...
    def process_item(self, item, spider):
        ...
        # 写sql语句 插数据，没有表的话要先在数据库创建
        sql = """INSERT INTO base_city_landmark(city, county, landmark, lng, lat, comprehension, city_level) VALUES ("%s","%s","%s","%s","%s","%s","%s") """ % (item['city'],item['county'],item['landmark'], item['lng'], item['lat'], item['comprehension'], item['city_level'])
